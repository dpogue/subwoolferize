...
const JeelizThreeGlassesCreator = function(spec){
  ...
  new THREE.BufferGeometryLoader().load(spec.lensesMeshURL, function(glassesLensesGeometry){
    ...
    const mat = new THREE.MeshBasicMaterial({
      opacity: 0.85,
      color: new THREE.Color().setHex(0x111111),
      transparent: true,
      fog: false
    });
  ...
  // woolf mask:
  new THREE.BufferGeometryLoader().load(spec.maskMeshURL, function(maskGeometry) {
    maskGeometry.computeVertexNormals();
    const mat = new THREE.MeshBasicMaterial({
      opacity: 1,
      color: new THREE.Color().setHex(0xffd400),
      transparent: false,
      fog: false
    });

    const maskMesh = new THREE.Mesh(maskGeometry, mat);
    threeGlasses.add(maskMesh);
  });

  // teeth:
  new THREE.BufferGeometryLoader().load(spec.teethMeshURL, function(teethGeometry) {
    ...
    const teethMesh = new THREE.Mesh(teethGeometry, mat);
    threeGlasses.add(teethMesh);
  });
